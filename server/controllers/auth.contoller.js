...
export const login = async (req, res) => {  //funcion asincrona para registrar
    ...
    try {
        ...
        if (!userFound) return res.status(400).json({ message: "Usuario no encontrado" }); //si no lo encuentra error
        
        const isMatch = await bcrypt.compare(password, userFound.password); //comparar la contraseña ingresada con la guardada
        if (!isMatch) return res.status(400).json({ message: "Contraseña incorrecta" }); //si no coincide error
